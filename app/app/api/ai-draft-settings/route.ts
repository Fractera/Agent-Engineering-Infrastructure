...
import type { DraftMode, DraftTier } from "@/lib/ai-draft/draft-format"
...
// mode, target?, tier?, mutating? }. target null = a brand-new record (orange + req);
// target set = a supplement/replace overlay on a real original (black name + req badge).
// tier/mutating are the MCP access fields (§8.3) — mcp-only, owner/true by default. Same
// write path for the human UI and an agent.
export async function POST(req: NextRequest) {
  const { agent, kind, name, mode, target, tier, mutating } = await req.json()
  ...
  const t: DraftTier = tier === "public" || tier === "user" || tier === "owner" ? tier : "owner"
  const mut = typeof mutating === "boolean" ? mutating : true
  const draft = await createDraft(String(agent), kind, String(name).trim(), m, target ? String(target) : null, t, mut)
