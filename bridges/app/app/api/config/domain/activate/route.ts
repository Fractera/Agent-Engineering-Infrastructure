...
/**
 * 🔒 СЛУЖБЫ, ЧИТАЮЩИЕ ФЛАГ РЕЖИМА, ПЕРЕЗАПУСКАЮТСЯ ЯВНО И С ОБНОВЛЕНИЕМ
 * ОКРУЖЕНИЯ (найдено на живом сервере 2026-08-17).
 *
 * ЧТО БЫЛО НЕ ТАК. Здесь стоял только `pm2 reload all`. Файлы окружения
 * переписаны, флаг на диске стал `false` — а служба данных продолжала работать
 * со СТАРЫМ значением `true` в памяти: `dotenv` читает файл ровно один раз при
 * старте процесса, а `reload` не обязан этот процесс поднять заново.
 *
 * Цена ошибки — не косметическая. `FRACTERA_IP_NODOMAIN_MODE=true` означает
 * ОБХОД АВТОРИЗАЦИИ: `POST /db/migrate` слоя данных выполняет произвольный SQL
 * над базой клиента без ключа и с неверным ключом. Проверено на сервере
 * владельца: обе попытки вернули `{"ok":true}` с настоящими данными, пока
 * панель показывала защищённый режим включённым.
 *
 * Дефект молчалив вдвойне: снаружи порт закрыт брандмауэром, панель зелёная,
 * файл на диске правильный. Единственный, кто знает правду, — процесс.
 *
 * Поэтому: сначала ИМЕННОЙ `restart --update-env` тем службам, чья
 * авторизация зависит от флага, и только потом общий `reload` для остальных.
 * Порядок обязателен — общий `reload` перезапускает и саму панель, обрывая
 * эту цепочку на середине.
 */
function pm2ReloadAllDetached(delayMs = 800): void {
  setTimeout(() => {
    const child = spawn(
      "sh",
      ["-c", `${restartAuthAndData()}; pm2 reload all`],
