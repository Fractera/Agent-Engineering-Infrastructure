...
export async function POST(req: NextRequest) {
  ...
  // 🔒 ФОРК НЕ ОТВЯЗЫВАЕТСЯ, А СТАРТОВЫЙ ШАБЛОН ОТВЯЗЫВАЕТСЯ — И ЭТО ОДИН
  // ВЫКЛЮЧАТЕЛЬ НА ДВА РАЗНЫХ СЛУЧАЯ (75-3, переворот решения 35-2).
  //
  // Форк — репозиторий ЧЕЛОВЕКА. Стереть его remote значит отрезать его от
  // собственной работы: некуда пушить, неоткуда тянуть. Раньше сюда приезжал
  // ЧУЖОЙ проект-донор, и отвязка была верна; модель сменилась, решение
  // перевернулось.
  //
  // Стартовый шаблон принадлежит нам, человеку он не принадлежит, и его история
  // в проекте человека — мёртвый груз (плюс мелкий клон отправить нельзя).
  // Поэтому возврат шаблона отвязку сохраняет.
  const result = restore
    ? restoreStarter(SLOT_DIR, { token })
    : replaceSlotContents(repoUrl, SLOT_DIR, { token, detach: false });
