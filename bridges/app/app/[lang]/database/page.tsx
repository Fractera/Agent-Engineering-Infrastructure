// Раздел «database» (шаг 501, каркас). Серверная страница: берёт слова из словаря
...
import { DatabasePanel } from "./_components/database-panel";

export default async function DatabasePage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const page = s.pages["database"];
