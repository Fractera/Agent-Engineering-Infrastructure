// Документ разработки «doc-architecture» (шаг 501, слой «Документы»).
//
...
const DOC_KEY = "doc-architecture" as const;

export default async function DocPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const d = s.docs;
  const page = s.pages["doc-architecture"];
  const state = readDoc(DOC_KEY);
  const kind = DOC_KIND[DOC_KEY];

  return (
    <PageShell lang={lang} slug="doc-architecture" s={s} title={page.title} hint={page.hint}>
      <div className="mb-2">
        <DocKindBadge
          kind={kind}
