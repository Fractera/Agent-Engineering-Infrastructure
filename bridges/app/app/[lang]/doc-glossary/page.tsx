// Документ разработки «doc-glossary» (шаг 501, слой «Документы»).
//
...
const DOC_KEY = "doc-glossary" as const;

export default async function DocPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const d = s.docs;
  const page = s.pages["doc-glossary"];
  const state = readDoc(DOC_KEY);
  const kind = DOC_KIND[DOC_KEY];

  return (
    <PageShell lang={lang} slug="doc-glossary" s={s} title={page.title} hint={page.hint}>
      <div className="mb-2">
        <DocKindBadge
          kind={kind}
