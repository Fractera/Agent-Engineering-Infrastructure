...
//
// 🔒 ЧИСЛО ЗДЕСЬ БОЛЬШЕ НЕ ПИШЕТСЯ СЛОВОМ, И ЭТО ТРЕТЬЯ ПРАВКА ОДНОГО КЛАССА.
// Строки порождаются из `SUBDOMAINS` с 2026-09-03, а число во ВВОДНОЙ ФРАЗЕ
// оставалось рукописным — и разошлось на следующем же поддомене (`memory`,
// 2026-09-10): экран просил «все пять» и показывал шесть. Теперь и оно
// порождается, подстановкой `{n}` в `dns-records.tsx`.
//
