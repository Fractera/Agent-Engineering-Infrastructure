...
export function PathChoice({ labels }: { labels: PathChoiceLabels }) {
  ...
  return (
    <div>
      {/* 🔒 ДВЕ КОЛОНКИ — ТРЕБОВАНИЕ ВЛАДЕЛЬЦА (2026-08-27): «use 2 column design
          for cards». Раньше здесь стояла одна колонка, и причина была честная:
          в колонке панели (48rem) пара колонок даёт каждому контейнеру ~21rem, и
          заголовок «Запуск проекта со стартового шаблона» ломался на три строки.
          Лечение — не отказ от двух колонок, а ШИРИНА: страница `project-start`
          просит у рамки широкий режим (`wide`), и на ней каждому контейнеру
          достаётся ~30rem. Вторая колонка появляется с `md`, ниже — друг под
          другом: на телефоне две колонки не бывают большими. */}
