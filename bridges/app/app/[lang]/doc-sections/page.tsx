// Документ разработки «doc-sections» — секции, дизайны и проекты на одном сервере (шаг 508).
// Логика общая с остальными документами: страница знает только свой ключ, файл
// читает lib/product-docs.ts, правит общий островок doc-editor.client.tsx.
...
const DOC_KEY = "doc-sections" as const;

export default async function DocPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const d = s.docs;
  const page = s.pages["doc-sections"];
  const state = readDoc(DOC_KEY);
  const kind = DOC_KIND[DOC_KEY];

  return (
    <PageShell lang={lang} slug="doc-sections" s={s} title={page.title} hint={page.hint}>
      <div className="mb-2">
        <DocKindBadge
          kind={kind}
