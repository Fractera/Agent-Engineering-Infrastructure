// Раздел «vector-memory» (шаг 501, каркас). Серверная страница: берёт слова из словаря
...
import { VectorMemoryPanel } from "./_components/vector-memory-panel";

export default async function VectorMemoryPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const page = s.pages["vector-memory"];
