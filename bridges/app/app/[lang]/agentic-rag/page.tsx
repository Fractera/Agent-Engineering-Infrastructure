// Раздел «agentic-rag» (шаг 501, каркас). Серверная страница: берёт слова из словаря
...
import { AgenticRagPanel } from "./_components/agentic-rag-panel";

export default async function AgenticRagPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const page = s.pages["agentic-rag"];
