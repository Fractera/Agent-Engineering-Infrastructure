  ...
  return (
    ...
    >
      ...
      {/* 🔒 СЕКЦИЯ ПРОДУКТОВ И ЗЕЛЁНОЕ ОБЕЩАНИЕ — ОДНО ВМЕСТО ДРУГОГО
          (владелец 2026-08-15). Обещание живёт, пока показывать нечего; как
          только есть что показать, на его месте встаёт то, что оно обещало.
          Держать оба значило бы рекламировать человеку то, что у него уже есть.

          🔒 УСЛОВИЕ «ЕСТЬ КЕЙСЫ» ОКАЗАЛОСЬ НЕВЕРНЫМ (найдено проверкой 2026-08-16).
          Владелец заводил второй продукт, у нового кейсов ещё не было — и секция
          продуктов исчезала целиком, вместе с единственным способом вернуться к
          первому. Человек оставался в новом продукте без выхода.

          Поэтому: два и больше продуктов — секция видна ВСЕГДА, чей бы продукт ни
          был открыт и пуст ли он. Один продукт — по-прежнему ждём кейсов, чтобы не
          показывать список из одной строки тому, кто ещё ничего не описал. */}
      {(products.length > 1 || gate.kind !== "missing") && (
        <ProductsSection
          products={products}
          current={product}
      ...
      {gate.kind === "missing" && products.length <= 1 && (
      ...
      {!questions && !seed ? (
        ...
        <div className="mt-2">
          ...
          <div className="mt-3">
          <IntroSetup
            // 🔒 КЛЮЧ ОБЯЗАТЕЛЕН, И БЕЗ НЕГО ЭКРАН ПОКАЗЫВАЛ ЧУЖИЕ ВОПРОСЫ
            // (нашёл владелец 2026-08-16, проверяя смену структуры).
            //
            // Список вопросов внутри редактора — состояние: владелец правит его
            // руками, поэтому он живёт в `useState`, засеянном из `suggested`.
            // Засев происходит РОВНО ОДИН РАЗ, при монтировании. Смена структуры
            // не размонтирует редактор: `router.refresh()` перерисовывает
            // серверное дерево, новые вопросы приезжают пропсом — а состояние
            // остаётся прежним. На экране вопросы посадочной страницы, в записи
            // продукта — SaaS, и расходятся они молча до перезагрузки страницы.
            //
            // Ключ из продукта и структуры делает смену РОЖДЕНИЕМ нового
            // редактора: React размонтирует прежний и засевает новый свежими
            // вопросами. Правки внутри одной структуры при этом сохраняются —
            // ключ не меняется, пока не сменился выбор.
            //
            // Синхронизация через `useEffect` была бы хуже вдвойне: лишняя
            // перерисовка и стёртые правки владельца при любом обновлении
            // страницы, а обновляет её здесь каждое действие.
        ...
        <div className="mt-3">
          ...
          <IntroQuestions
            // 🔒 ТОТ ЖЕ КЛЮЧ И ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО У РЕДАКТОРА ВОПРОСОВ ВЫШЕ
            // (2026-08-16). Ответы засеваются массивом по числу вопросов ровно
            // при монтировании; сменилась структура — вопросов стало другое
            // число, а массив ответов остался прежней длины. Здесь это опаснее,
            // чем у редактора: там были видны чужие вопросы, тут ответы молча
            // встали бы не к тем вопросам, и заметить это можно только по готовым
            // кейсам, когда переписывать надо всё.
            //
            // В ключ входит и САМ СПИСОК: вопросы меняет не только структура, но
            // и правка владельца в редакторе выше — она не трогает `product.type`.
