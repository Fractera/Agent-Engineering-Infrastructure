...
export default async function CustomFracteraRepoStepSix(
  ...
  const x = adoptStepSixStrings(lang);
  ...
  const locked = adoptLockedFor(lang, 6);
  if (locked) {
    return (
      <PageShell
        lang={lang}
  ...
  return (
    <PageShell
      ...
      slug="project-start"
      s={s}
      tail={[{ label: "custom-fractera-repo", href: base }, { label: "step-6" }]}
      title={x.pageTitle}
      hint={x.pageHint}
    >
      <StepSection
      ...
      >
        {done ? (
          <StepNav
            ...
            nextHref={adoptStepBuilt(7) ? `${base}/step-7` : undefined}
