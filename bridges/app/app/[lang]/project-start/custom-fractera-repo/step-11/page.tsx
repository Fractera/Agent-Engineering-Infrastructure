...
export default async function CustomFracteraRepoStepThirteen(
  ...
  const locked = adoptLockedFor(lang, 11);
  if (locked) {
    return (
      <PageShell
        lang={lang}
        slug="project-start"
        s={s}
        tail={[{ label: "custom-fractera-repo", href: base }, { label: "step-11" }]}
        title={x.pageTitle}
        hint={x.pageHint}
      >
        <StepLocked
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          {marked ? (
            <StepNav
              prevHref={adoptStepBuilt(10) ? `${base}/step-10` : undefined}
              nextHref={adoptStepBuilt(12) ? `${base}/step-12` : undefined}
