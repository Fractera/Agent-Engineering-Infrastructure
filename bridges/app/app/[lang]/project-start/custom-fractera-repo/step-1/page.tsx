  ...
  return (
    ...
    >
      <StepSection
        ...
        // ✗ 🔒 ССЫЛКИ ЗДЕСЬ НЕ БЫЛО, А ПОДПИСЬ ДЛЯ НЕЁ БЫЛА — то есть строка слов
        // существовала и никуда не вела (найдено сверкой с первым путём 2026-08-31).
        // Шаг говорит «идите на GitHub и нажмите Fork» и не давал туда дороги.
        //
        // 🔒 ВЕДЁТ НА ОБЪЯСНЕНИЕ, А НЕ НА КОНКРЕТНЫЙ ФОРК, И ЭТО ЧЕСТНО: какой
        // проект человек хочет форкнуть, знает только он. У первого пути ссылка
        // ведёт на `github.com/new`, потому что там репозиторий создаётся с нуля и
        // адрес один на всех; здесь такого адреса не существует.
        //
        // 🔒 ЗАКОН 28-11: ссылка-действие — ГЛАВНАЯ ЧАСТЬ ИНФОРМАЦИИ шага, а не
        // второе его действие. Она ведёт наружу; действие человек делает здесь.
        link={{
          href: "https://docs.github.com/get-started/quickstart/fork-a-repo",
          label: x.linkLabel,
        }}
        // 🔒 СНИМОК ЧУЖОГО ЭКРАНА — прислан владельцем 2026-08-31 для этого шага.
        // Страница форка на GitHub: человек видит её раньше, чем откроет, и узнаёт
        // в лицо. Ставится тем же элементом `StepShot`, что на первом пути, и
        // рисуется, только если у слов есть обе части — что на снимке и чей он.
        shot={
          x.shotAlt && x.shotCaption
            ? { src: "/images/launch/step-1-fork.png", alt: x.shotAlt, caption: x.shotCaption }
            : undefined
        }
