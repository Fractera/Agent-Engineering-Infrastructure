// Раздел «login-methods» (шаг 501, каркас). Серверная страница: берёт слова из словаря
...
import { LoginMethodsPanel } from "./_components/login-methods-panel";

export default async function LoginMethodsPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const page = s.pages["login-methods"];
