// Документ разработки «doc-antipatterns» (шаг 501, слой «Документы»).
//
...
const DOC_KEY = "doc-antipatterns" as const;

export default async function DocPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const s = getAdminStrings(lang);
  const d = s.docs;
  const page = s.pages["doc-antipatterns"];
  const state = readDoc(DOC_KEY);
  const kind = DOC_KIND[DOC_KEY];

  return (
    <PageShell lang={lang} slug="doc-antipatterns" s={s} title={page.title} hint={page.hint}>
      <div className="mb-2">
        <DocKindBadge
          kind={kind}
