...
export function publicSiteUrl(): PublicSite {
  if (isSecureMode()) {
    ...
    //
    // Провалиться отсюда к `http://<IP>:3000` нельзя: на сервере, работающем за
    // сертификатом, наружу пропускается только nginx, и порт 3000 закрыт. Панель
    // назвала бы адрес, по которому ничего нет, а человек решил бы, что сломан
    // его сервер. Молчание честнее догадки: текст шага без адреса читается
    // связно, с неверным адресом — уверенно врёт.
    //
    // ✗ ЧЕСТНАЯ ИСТОРИЯ ЭТОЙ ВЕТКИ, чтобы её не «улучшили» обратно. Я завёл её,
    // измерив на живом сервере пустой `custom_domain`, — и вывод был НЕВЕРЕН:
    // `sqlite3` в системе не установлен, поэтому мой зонд молчал не потому, что
    // домен пуст, а потому, что спрашивать было нечем. Домен там `aifa.dev`, и
    // страница берёт его. Ветка всё равно нужна — сервер без визарда домена
    // существует, — но родилась она из третьей причины нуля: негодного зонда.
