...
export const FOLDER_DOCS: Record<string, string> = {
  ...
  // 🔒 ИМЯ СЕРВЕРА НАЗЫВАЕТСЯ ТОЧНО. Здесь стояло `development-steps` — так
  // сервер звался до того, как получил инструменты кейсов и стал
  // `fractera-project`. Агент читает эту строку как адрес двери; названная
  // неверно, она отправляет его искать инструменты, которых нет, и он честно
  // доложит, что шагов не существует.
  "doc-steps": `MCP fractera-project → ${STEPS_SOURCE}`,
};
...
/**
 * 🔒 ДОКУМЕНТЫ, КОТОРЫЕ ЕЩЁ НЕ ОТКРЫТЫ (владелец 2026-08-11).
 *
 * Раздел виден, документ пуст, выключатель на месте — но включить его нельзя:
 * попытка отвечает сообщением, что возможность откроется в ближайшем обновлении.
 *
 * ЗАЧЕМ ИМЕННО ТАК, а не «спрятать раздел». Спрятанное невозможно ждать: владелец
 * не знает, что возможность существует, и строит обход. Видимый раздел с честной
 * меткой «опция в разработке» говорит и о том, что она будет, и о том, что её
 * сегодня нет, — одним взглядом.
 *
 * 🔒 ЗАПРЕТ ЖИВЁТ НА СЕРВЕРЕ, а не в интерфейсе: маршрут сохранения отказывает
 * сам. Интерфейсный запрет обходится одним `curl`, и тогда в инструкции окажется
 * закон о механизме, которого нет.
 */
export const IN_DEVELOPMENT = new Set<string>(["doc-parallel-routing"]);
...
export const COMMAND_DEFAULTS: Record<string, Partial<Record<CommandVerb, Record<string, string>>>> = {
  ...
  // включён по умолчанию, и просить включить его незачем — просят предъявить то,
  // что он требует, не дожидаясь конца шага.
  "doc-testing": {
    activate: { en: "give me the proofs", ru: "предоставь доказательства" },
  },
  // «Запиши передачу ПРЯМО СЕЙЧАС» (владелец 2026-08-11) — не дожидаясь порога
  // заполнения окна. Владелец видит признаки конца окна раньше механизма: он
  // собирается закрыть ноутбук, сменить чат или отойти.
  //
  // ⚠️ ПОКА ДОКУМЕНТ ВЫКЛЮЧЕН, КОМАНДЫ В ИНСТРУКЦИИ НЕТ. Управляемая область
  ...
  // Стандарты кода. `activate` здесь значит «сверься со стандартом ПРЯМО СЕЙЧАС,
  // до строчки кода» — документ включён, включать нечего; `add` пополняет его
  // словом владельца, а не догадкой агента.
