...
//
// 🪦 ДО 2026-08-31 КЛЮЧ НАЗЫВАЛСЯ `donor-url` И ЗНАЧИЛ ДРУГОЕ: «чужой проект,
// который я хочу взять». Владелец назвал ту модель неверной — человек делает
// ФОРК и с первой минуты работает в СВОЁМ репозитории. Новое значение — «мой
// форк», то есть другой факт о другом действии. Переименование, а не
// переиспользование: один ключ на два разных факта объявил бы сделанным то,
// чего человек не делал (шаг 75, решение владельца «Форк вместо донора»).
//
// ✗ Цена названа заранее: на уже развёрнутых серверах `USER_FLOW_DONOR_URL`
// остаётся осиротевшим. Это верно — он отвечал на вопрос, которого больше не
// задают; чистить чужой `.env.local` ради опрятности не станем.
