...
// Какие части при загрузке ЗАМЕНЯЮТ то, что есть, а какие добавляют (шаг 501).
//
// Это МАШИННЫЙ факт о поведении, а не слово, поэтому он живёт здесь, рядом со
// списком частей, а не в файле переводов: там его могла бы «перевести» внешняя
// модель, и предупреждение оранжевым перестало бы совпадать с реальностью.
// Машинная приёмка языков (`npm run check:i18n`) поймала именно это и была права.
