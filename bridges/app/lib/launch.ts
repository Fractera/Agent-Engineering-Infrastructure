...
// машинный шаг читает ТОТ ЖЕ факт, которым живёт сама способность: репозиторий —
// `USER_GITHUB_REPO_URL` плюс своя отметка проверки, ключ доступа — файл пары
// (`keyIssued()`), а человеческие шаги — снимаемые отметки в том же файле, у того
// же единственного писателя.
//
// ✗ Эта шапка обещала, что шаги читают и `USER_GITHUB_VERIFIED_AT`. После правки
// 25-3 это перестало быть правдой, и ревизия 25-8 поймала расхождение: комментарий
// пережил код на пять подшагов. Ключ жив — его чистит сброс со связью, — но ни
// один шаг им больше не закрывается.
//
 ...
 */
/**
 * Шаги, чья отметка ЖИВЁТ НЕ ЗДЕСЬ, а в уже существующем ключе окружения.
 *
 * 🔒 ЗАИМСТВОВАТЬ МОЖНО ТОЛЬКО ТОТ КЛЮЧ, КОТОРЫЙ ОЗНАЧАЕТ ТО ЖЕ САМОЕ. Галочка
 * мастера «поставил Claude Code» и галочка вкладки инструментов разработки — один
 * факт, у него один дом, и второй ключ развёл бы их через неделю.
 *
 * ✗ Оплачено 2026-08-27, поймано измерением в 25-3. Здесь стояло
 * `key: "USER_GITHUB_VERIFIED_AT"` — отметка, которую пишет проверка
 * РЕПОЗИТОРИЯ. Замер показал: достаточно подключить репозиторий, и второй шаг —
 * «получить ключ безопасности» — загорается зелёным, хотя ключа на диске нет.
 * Мастер поздравлял человека с тем, чего он не делал, и вёл дальше без ключа.
 */
...
export type LaunchState = {
   ...
   *
   * 🔒 Правило раскрытия: показан РОВНО ОДИН шаг — этот. Пройденные свёрнуты в
   * зелёные бусины верёвочки, будущих не видно вовсе. Решение владельца дословно:
   * «до того пока человек не пройдёт первый шаг, ему не нужно показывать никакие
   * другие шаги». Прежняя редакция этой строки обещала `0..current` включительно
   * — то есть все пройденные развёрнутыми; исправлено в 25-2 по факту приёмки.
   */
  current: number;
...
function stepDone(id: LaunchStepId): boolean {
  ...
  // 🔒 У КЛЮЧА ДОСТУПА НЕТ ОТМЕТКИ, И НЕ ДОЛЖНО БЫТЬ. Его правда — файл пары
  // ключей и строка в `authorized_keys`; `keyIssued()` читает именно их. Отметка
  // в окружении была бы вторым источником того же факта и разошлась бы с ним в
  // первый же отзыв ключа: человек отозвал доступ, а мастер показывает зелёное.
  // Так шаг честно ОТКРЫВАЕТСЯ обратно, когда ключа не стало.
  if (id === "key") return keyIssued();
