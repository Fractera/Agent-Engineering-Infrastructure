...
export function defaultSurface(type: ProjectTypeId): ProductSurface {
  ...
  // 🔒 «МОЗГ КОМПАНИИ — БИЗНЕС» ЗАКРЫТ БЕЗУСЛОВНО (2026-08-16). Он сводит в одно
  // место переписку с клиентами, счета, записи звонков и зарплатные данные —
  // то есть коммерческую тайну и чужие персональные данные разом. Публичный
  // адрес здесь означал бы не утечку, а публикацию.
  // 🔒 АТТЕСТАЦИЯ ЗАКРЫТА: в ней данные сотрудников — кто, когда и с каким
  // результатом сдавал, — и экзаменационные билеты, открытый доступ к которым
  // обесценивает саму проверку знаний.
  if (
    type === "competitors" || type === "company-brain" || type === "business-brain"
    || type === "crm" || type === "attestation"
  ) return "private";
  // 🪦 «АГЕНТНАЯ АВТОМАТИЗАЦИЯ» БОЛЬШЕ НЕ БЕЗЭКРАННАЯ (владелец 2026-08-16).
  // Она была `headless`, пока означала работника, живущего в каналах и
  // расписании: экрана у него не было вовсе. Направление переопределено —
  // теперь это многошаговое исследование, где человек ставит цель, СМОТРИТ ход
  // работы и читает готовый отчёт. Экран есть, и он главный.
  //
  // Закрытый по умолчанию, а не публичный: агент ходит во внутренние системы и
  // сводит то, что нашёл, в документ; открытый по ошибке адрес означал бы чужую
  // аналитику наружу. Правило умолчания то же, что у CRM.
  if (type === "agents") return "private";
  return "public";
}
...
export function productPaths(product: Pick<Product, "id" | "route">) {
  ...
  return {
    // 🔒 КОРНЕВОЙ ПРОДУКТ ЖИВЁТ В `(publicLayer)`, А НЕ В ВЫДУМАННОЙ `(root)`
    // (найдено сквозным прогоном 2026-08-17, агент пошёл строить по контракту).
    //
    // Здесь стояло `app/[lang]/(root)/` — красивая группа маршрутов, которой в
    // шаблоне слота НЕ СУЩЕСТВУЕТ. Главная страница лежит в
    // `app/[lang]/(publicLayer)/page.tsx`, и это не случайность раскладки: две
    // группы, `(publicLayer)` и `(protectedLayer)`, отвечают на вопрос «зависит
    // ли показанное от того, кто смотрит», и каждый маршрут обязан быть в одной
    // из них — маршрут вне групп гейт называет «незаданным вопросом».
    //
    // Агент, следующий контракту буквально, создал бы третью группу рядом с
    // двумя объявленными и вынес бы туда главную. Путь, обещанный в контракте и
    // отсутствующий на диске, хуже отсутствующего контракта: по нему идут.
    pages: root ? "app/[lang]/(publicLayer)/" : `app/[lang]/(publicLayer)/${segment}/`,
