...
| **Navigation buttons** | control panel on \`:3002\` → Top menu | Which pages the top menu links to, their order, and which collapse into a dropdown group. The app reads them from its config at render, so a change appears without a rebuild. **Anything menu-shaped → load the \`manage-top-menu\` skill and run \`npm run read:menu\` first** — the buttons live outside git, so the repository alone shows an empty header and invites you to build a second one. \`npm run check:menu\` fails on that second header. |
