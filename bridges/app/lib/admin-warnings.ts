...
export function collectWarnings(): AdminWarning[] {
  ...
  if (!casesReady) {
    // 🔒 ВЕДЁТ ТУДА, ГДЕ РАБОТА, А НЕ ТУДА, ГДЕ ОБЪЯСНЕНИЕ (2026-08-18).
    // Кейсы переехали в группу «Продукты»; документ остался документом. Тревога,
    // приводящая на страницу без единой кнопки, читается как поломка панели.
    out.push({ id: "use-cases", level: "blocking", slug: "products" });
  }
