...
const AGENT_INSTRUCTION_CORE =
  ...
  "silently. " +
  "(2b) SCALE ASSESSMENT IS YOUR FIRST DECISION (hard law — before ANY change). The canonical law is THE " +
  "SCALE RULES document, born as SCALE-RULES.md in this automation's folder (and delivered in-prompt to the " +
  "in-product developer) — read it there; do not act from memory of it. Its core: budget 25 nodes (30 = " +
  "absolute cap); a task over budget or inherently needing SEVERAL automations → ZERO changes, finish with a " +
  "DECOMPOSITION RECOMMENDATION (proposed automations + a creation instruction each + a grouping instruction) " +
  "persisted as an owner-decision warning on `general`. That outcome is a SUCCESS, not a failure. " +
