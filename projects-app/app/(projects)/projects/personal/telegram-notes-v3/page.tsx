...
export default async function Page() {
  ...
  // ТОЛЬКО ВИДИМЫЕ узлы (308.9): настраивают ключи каналов, которые автоматизация РЕАЛЬНО использует
  // (раскрыты на холсте). Скрытый канал не задействован — его ключи не должны засорять форму настроек
  // (иначе всплывал бы, напр., TELEGRAM_ALLOWED_CHAT_ID телеграм-бота, которого в этой автоматизации нет).
  const nodeKeys = allNodes(graph.nodes).filter((n) => n.state === "visible").flatMap((n) => n.envKeys.map((k) => k.name));
