...
## The asymmetry — why this group alone has no vocabulary

Every other layer is FINITE and therefore closed by an inventory:

| Layer | Closed by | Frozen? |
|---|---|---|
| input | the vocabulary of channels | yes |
| intent | the vocabulary of request classes | yes |
| output | the vocabulary of destinations | yes |
| evolution | the vocabulary of self-change scopes | yes |
| **middle** | **nothing — the logic of the world does not enumerate** | **no: it is BUILT** |

That is not an omission to be corrected. Doors are revealed; the middle is written. It has no vocabulary,
no derived function names and no quota beyond one node of each kind, and it is closed by KNOWLEDGE instead:
the corpus of node patterns, plus the law of three outcomes (`nodes.md`).

## It is designed LAST, and that is a rule

The middle's shape follows from what reaches it, so it cannot be designed before the front is settled.
Of the eleven request classes only SIX route here — `record-given`, `read-own`, `fetch-external`,
`composite`, `control`, `continuation`. The other five (`self-describe`, `refuse`, `small-talk`,
`incomplete`, `unclaimed`) are answered without the middle at all, and building work for them is building
for requests that will never arrive.

Practical consequence: a run enters the middle already CLASSIFIED. `ctx.intentClass` says which class
claimed it, and a middle node may rely on that instead of guessing again. A middle node that re-classifies
the request is doing the front's work in the wrong layer.
...
The other layers are reused as INVENTORY — the same doors and the same classes stand in every automation.
The middle is reused as KNOWLEDGE: not the nodes themselves, but the patterns behind them. A pattern from
the corpus tells you the shape of a solution; you write your own code from it and record its `lineage` on
your node. Pasting a foreign node's file is not reuse — it imports that folder's contract, its table names
and its assumptions about the output layer (step 310, `middle.custom.md`).
