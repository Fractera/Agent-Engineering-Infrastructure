 ...
 */
/**
 * 🔒 ПРИЗНАК — ОТСЫЛКА, А НЕ ДЛИНА (332.F, дефект найден владельцем на живом экране).
 *
 * Здесь стояло «три слова и короче — дать контекст». Но короткое сообщение бывает совершенно
 * самодостаточным: «Эйфелева башня» — это ИМЯ, а не отсылка. Получив контекст предыдущей болтовни, слой
 * намерения читал его как продолжение разговора и объявлял `unclaimed` — главный сценарий сборки не
 * запускался вовсе, а ассистент отвечал «я МОГУ открыть для вас Эйфелеву башню» вместо того, чтобы
 * открыть. В чистом чате та же фраза уверенно опознаётся как добыча: губил именно контекст.
 *
 * Значит признак должен называть ровно то, ради чего контекст и нужен — ССЫЛКУ НА СКАЗАННОЕ РАНЬШЕ.
 * Список отсылок конечен и потому закрыт: местоимения, согласия, порядковые указатели, относительные
 * дни. Нет отсылки — сообщение стоит само за себя, и контекст ему только вредит.
 */
const ANAPHORA = new Set([
  "да", "нет", "ок", "давай", "это", "то", "он", "она", "оно", "они", "там", "тогда", "туда", "тоже",
  "первое", "второе", "третье", "первый", "второй", "третий", "оба", "все",
  "завтра", "сегодня", "вчера", "потом", "позже", "сейчас",
  "yes", "no", "ok", "okay", "sure", "it", "that", "this", "them", "there", "then", "too",
  "first", "second", "third", "both", "all", "tomorrow", "today", "yesterday", "later", "now",
]);
