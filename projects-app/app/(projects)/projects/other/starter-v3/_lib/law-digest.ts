...
export function lawDigest(core?: Automation): LawDigest {
  return {
    ...
    laws: [
      ...
      "🔒 TOAST IS THE DEFAULT DESTINATION, not merely the run's outcome channel: ANYTHING the automation must tell the human goes out through a connected channel, and when NO channel is connected or active it goes to the toast — «nobody was told» is not an expressible state. This covers deferred announcements too (a due calendar moment: no active integration → a toast row, marked `deliveredAt` for exactly-once). Narrow on purpose: the fallback fires only when there is NO active channel; a channel that exists and FAILED must retry, never be replaced by a toast. Any new path that notifies a human declares this fallback — a notification with no channel is a defect, not a configuration (step 328)",
      ...
      "🔒 SPEECH IS A LAYER, NOT A MIDDLE NODE (step 312): one node (`converse`, kind `speech`) writes `ctx.reply` — the ONE answer every open channel delivers, and no output node composes speech any more. It stands after the front and the middle because the answer is CONTENT, not a destination, and because FIVE of the eleven request classes never reach the middle. A class declares only the KIND of answer (`speechAct`: refuse · greet · describe-self · ask · not-understood) and its subject; the wording, in the chat's language, belongs to speech. Exactly ONE speech node exists: two voices would mean two places where the answer is assembled",
      "🔒 THE DIALOGUE PLANE — the axis one run does not have: recent messages, the outstanding question and the chosen language live with the INTERLOCUTOR (`telegram:<id>`, `email:<from>`, `panel`) and outlive runs. The ENGINE attaches it once per run as soon as the doors name the interlocutor; ANY layer may read it; only the SPEECH layer writes it. Without it the class `continuation` could never claim — the question died with its run (step 312.3)",
      "🔒 ABILITIES, ADDRESSES AND ACCESS ARE DERIVED FROM THE CORE on every run, never written into the behaviour text: what the build can do (open channels + middle steps), where it lives (passport `publicUrl` + the cockpit address taken from the request) and who may see it (`passport.access`). A hand-written capability list ROTS — it once promised receipts and maps in a build that only captured messages. The derived lines OUTRANK the behaviour instruction, and inventing beyond them is forbidden (step 312.4/312.7)",
      "🔒 LANGUAGE IS THREE DIFFERENT THINGS: (1) the chat's own choice («let's speak Ukrainian») — strongest, lives in the dialogue plane; (2) the CHAT LANGUAGE setting (`assistant` tab, `language.chat`) — set and non-empty ⇒ always used; (3) the platform default — ONLY when the human has written nothing yet and the automation speaks first. If he has written, HIS language beats the default: he showed it by writing (step 312.7)",
      "🔒 A ROW LEADS TO THE ENTITY: clicking a row — or a marker, or any object a tab draws — opens the ONE shared entity drawer (`_components/shared/entity-drawer.client.tsx`, opened by the shared table itself) with the database record and every facet linked to it: objects, memory, places, events. Its data is `api/rows?table=&id=&linked=1`; objects inside it are shown by `media-viewer` and by nothing else; width, overlay and animation belong to the primitive, not to a tab. A tab that builds its own «details» view is a defect — the dashboard carried one and knew nothing of a row's neighbours (step 328). A journal row has no entity, and the drawer says so in words. THE ROW CLICK BELONGS TO THE ENTITY: any other per-row action (editing, a channel, settings) gets its own visible control — one gesture cannot mean two things",
