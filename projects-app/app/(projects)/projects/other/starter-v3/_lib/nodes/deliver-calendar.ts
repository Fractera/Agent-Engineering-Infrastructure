...
// integrations.
//
// ВРЕМЯ СОБЫТИЯ (шаг 307.6): разобранная серединой дата `ctx.when` (её кладёт узел `resolveMoment`)
// важнее момента захвата — так названный момент встаёт на своё место, а не на «сейчас». Нет `when` →
// момент захвата (`at`). `ctx.needsWhen` (момент нужен, но не определён) → честный отказ «когда?» на
