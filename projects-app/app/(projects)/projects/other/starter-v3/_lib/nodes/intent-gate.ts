...
import { needsContextToClassify } from "../components/conversation/context";
...
export async function guessClass(ctx: NodeCtx): Promise<string> {
  ...
  if (!text) return "";
  // 🔒 КЛАСС ЧИТАЕТСЯ В КОНТЕКСТЕ — НО ТОЛЬКО КОГДА СООБЩЕНИЕ БЕЗ НЕГО НЕПОЛНО (330.2, сужено в 330.2R).
  //
  // Зачем контекст: короткое «а второе?» опознать нечем — не потому, что модель слаба, а потому что мы не
  // давали ей того, что видит любой человек: предыдущую реплику.
  //
  // Почему НЕ ВСЕГДА (регресс, пойманный живьём): полная фраза опознаётся сама, а лишняя история ей
  // ВРЕДИТ — модель видела прошлые вердикты и повторяла их, разговор залипал в «не понял». Поэтому
  // контекст даётся только при висящем вопросе или очень короткой реплике, и в нём только слова ЧЕЛОВЕКА
  // (движок отдаёт уже очищенный срез).
  const pendingKind =
    ctx.pendingQuestion && typeof ctx.pendingQuestion === "object"
      ? String((ctx.pendingQuestion as { kind?: unknown }).kind ?? "").trim()
      : "";
  const plane = needsContextToClassify(text, Boolean(pendingKind))
    ? String(ctx.recentDialogBrief ?? "").trim()
    : "";
  const key = runId || `${plane} ${pendingKind} ${text}`;
  ...
  try {
    const answer = await askModel({
      system:
        ...
        `You may be given the person's earlier lines and a question the assistant is waiting an answer to. ` +
        `They exist ONLY to resolve what a short message refers to. Judge the LAST message on its own ` +
        `meaning: never repeat an earlier verdict, and never let a previous failure to understand make this ` +
        `message unclear too.`,
      user: [
        plane ? `Earlier lines from the same person:\n${plane}` : "",
        pendingKind ? `The assistant is waiting for an answer to its own question (${pendingKind}).` : "",
        `Message: ${text}`,
      ]
        .filter(Boolean)
        .join("\n\n"),
