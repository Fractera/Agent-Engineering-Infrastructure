...
//
// Раскрывашка на shadcn-`Accordion` (обязательность shadcn во v2, решение A). Компонент остаётся серверным:
// аккордеон — клиентский островок, а его содержимое рендерится на сервере и передаётся детьми. Живёт в
// кокпите владельца (admin-поверхность), а он свободен от канона no-JS. Для каждой таблицы: её хранилище,
// объявленные колонки и отправка задания в разработку — тем же общим компонентом, что и у пульта.
