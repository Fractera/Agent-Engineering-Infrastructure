...
export default function History({ entity, lang, heading = true }: { entity: Entity; lang: string; heading?: boolean }) {
  ...
  return (
    <section data-dashboard-entity={entity.cuid} className="min-w-0 space-y-3">
      {/* Имя таблицы рисуем ТОЛЬКО когда оно не стоит уже в шапке вложенного аккордеона: две одинаковые
          строки подряд — это не заголовок, а шум. */}
      {heading ? (
