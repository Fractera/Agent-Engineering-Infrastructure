...
export type WarningStrings = {
  ...
  // Step 253 — kind owner-decision (the develop agent's DECOMPOSITION plan) gets its OWN button labels:
  // the owner acts on the plan themselves, no Hermes is involved, so "instruction for Hermes" was wrong.
  decisionReveal: string;    // "Show the plan" — toggles the plan block
  decisionCopy: string;      // "Copy the plan"
  copied: string;
...
export const WARNING_I18N: Record<string, WarningStrings> = {
  en: {
    ...
    framingDecision: "This automation turned out too big to develop as one — the agent prepared a step-by-step plan below. Read it and follow the steps: create the listed automations and, at the end, delete this one. You can also write a decision in the field below to repeat the development round.",
    ...
    decisionReveal: "Show the plan", decisionCopy: "Copy the plan",
  ...
  ru: {
    ...
    framingDecision: "Эту автоматизацию нельзя разработать как одну — она слишком большая, поэтому агент подготовил пошаговый план ниже. Прочитайте его и выполните шаги: создайте перечисленные автоматизации, а в конце удалите текущую. При необходимости можно написать решение в поле ниже — этап разработки повторится.",
    ...
    decisionReveal: "Показать план", decisionCopy: "Скопировать план",
  ...
  es: {
    ...
    framingDecision: "Esta automatización resultó demasiado grande para desarrollarla como una sola — el agente preparó un plan paso a paso abajo. Léalo y siga los pasos: cree las automatizaciones indicadas y, al final, elimine esta. También puede escribir una decisión en el campo de abajo para repetir la ronda de desarrollo.",
    ...
    decisionReveal: "Mostrar el plan", decisionCopy: "Copiar el plan",
  ...
  fr: {
    ...
    framingDecision: "Cette automatisation s'est révélée trop grande pour être développée en une seule — l'agent a préparé un plan étape par étape ci-dessous. Lisez-le et suivez les étapes : créez les automatisations indiquées puis, à la fin, supprimez celle-ci. Vous pouvez aussi écrire une décision dans le champ ci-dessous pour répéter le tour de développement.",
    ...
    decisionReveal: "Afficher le plan", decisionCopy: "Copier le plan",
  ...
  it: {
    ...
    framingDecision: "Questa automazione è risultata troppo grande per svilupparla come una sola — l'agente ha preparato un piano passo passo qui sotto. Lo legga e segua i passaggi: crei le automazioni indicate e, alla fine, elimini questa. Può anche scrivere una decisione nel campo qui sotto per ripetere il giro di sviluppo.",
    ...
    decisionReveal: "Mostra il piano", decisionCopy: "Copia il piano",
  ...
  de: {
    ...
    framingDecision: "Diese Automatisierung erwies sich als zu groß, um sie als eine zu entwickeln — der Agent hat unten einen Schritt-für-Schritt-Plan erstellt. Lesen Sie ihn und folgen Sie den Schritten: Erstellen Sie die aufgeführten Automatisierungen und löschen Sie am Ende diese. Sie können auch unten eine Entscheidung schreiben, um die Entwicklungsrunde zu wiederholen.",
    ...
    decisionReveal: "Plan anzeigen", decisionCopy: "Plan kopieren",
  ...
  pt: {
    ...
    framingDecision: "Esta automação revelou-se demasiado grande para ser desenvolvida como uma só — o agente preparou um plano passo a passo abaixo. Leia-o e siga os passos: crie as automações indicadas e, no fim, elimine esta. Também pode escrever uma decisão no campo abaixo para repetir a ronda de desenvolvimento.",
    ...
    decisionReveal: "Mostrar o plano", decisionCopy: "Copiar o plano",
  ...
  pl: {
    ...
    framingDecision: "Ta automatyzacja okazała się zbyt duża, by rozwinąć ją jako jedną — agent przygotował poniżej plan krok po kroku. Przeczytaj go i wykonaj kroki: utwórz wymienione automatyzacje, a na końcu usuń tę. Możesz też wpisać decyzję w polu poniżej, aby powtórzyć rundę rozwoju.",
    ...
    decisionReveal: "Pokaż plan", decisionCopy: "Kopiuj plan",
  ...
  tr: {
    ...
    framingDecision: "Bu otomasyon tek bir otomasyon olarak geliştirilemeyecek kadar büyük çıktı — ajan aşağıda adım adım bir plan hazırladı. Okuyun ve adımları izleyin: listelenen otomasyonları oluşturun ve sonunda bunu silin. Geliştirme turunu tekrarlamak için aşağıdaki alana bir karar da yazabilirsiniz.",
    ...
    decisionReveal: "Planı göster", decisionCopy: "Planı kopyala",
  ...
  nl: {
    ...
    framingDecision: "Deze automatisering bleek te groot om als één te ontwikkelen — de agent heeft hieronder een stapsgewijs plan opgesteld. Lees het en volg de stappen: maak de genoemde automatiseringen en verwijder aan het eind deze. U kunt ook een beslissing in het veld hieronder schrijven om de ontwikkelronde te herhalen.",
    ...
    decisionReveal: "Toon het plan", decisionCopy: "Kopieer het plan",
