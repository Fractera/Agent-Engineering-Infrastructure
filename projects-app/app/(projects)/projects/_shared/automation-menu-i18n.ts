...
export type AutomationMenuStrings = {
  ...
  // Dashboard accordion split-view (step 243.4) — the button toggling one vs two side-by-side tables, and
  // the empty state before any table is configured.
  dashboardSingleView: string;
  dashboardTwoView: string;
...
export const AUTOMATION_MENU_I18N: Record<string, AutomationMenuStrings> = {
  en: {
    ...
    dashboardSingleView: "Single dashboard",
    dashboardTwoView: "Two dashboards",
    dashboardEmpty: "No tables configured yet. A dashboard holds any number of tables; each is declared in the project's `_data/config.ts` and rendered through the shared table standard — see the project README, \"The dashboard tables & columns standard\".",
  ...
  ru: {
    ...
    dashboardSingleView: "Один дашборд",
    dashboardTwoView: "Два дашборда",
    dashboardEmpty: "Таблицы ещё не настроены. Дашборд может содержать любое количество таблиц; каждая объявляется в `_data/config.ts` проекта и отрисовывается через общий стандарт таблиц — см. README проекта, раздел «The dashboard tables & columns standard».",
  ...
  es: {
    ...
    dashboardSingleView: "Panel único",
    dashboardTwoView: "Dos paneles",
    dashboardEmpty: "Aún no hay tablas configuradas. Un panel puede contener cualquier número de tablas; cada una se declara en `_data/config.ts` del proyecto y se renderiza mediante el estándar de tablas compartido — consulta el README del proyecto, «The dashboard tables & columns standard».",
  ...
  fr: {
    ...
    dashboardSingleView: "Tableau unique",
    dashboardTwoView: "Deux tableaux",
    dashboardEmpty: "Aucune table configurée pour l'instant. Un tableau de bord peut contenir un nombre quelconque de tables ; chacune est déclarée dans `_data/config.ts` du projet et rendue via le standard de tables partagé — voir le README du projet, « The dashboard tables & columns standard ».",
  ...
  it: {
    ...
    dashboardSingleView: "Dashboard singola",
    dashboardTwoView: "Due dashboard",
    dashboardEmpty: "Nessuna tabella ancora configurata. Una dashboard può contenere un numero qualsiasi di tabelle; ciascuna è dichiarata in `_data/config.ts` del progetto e renderizzata tramite lo standard di tabella condiviso — vedi il README del progetto, «The dashboard tables & columns standard».",
  ...
  de: {
    ...
    dashboardSingleView: "Ein Dashboard",
    dashboardTwoView: "Zwei Dashboards",
    dashboardEmpty: "Noch keine Tabellen konfiguriert. Ein Dashboard kann beliebig viele Tabellen enthalten; jede wird in der `_data/config.ts` des Projekts deklariert und über den gemeinsamen Tabellenstandard gerendert — siehe die README des Projekts, „The dashboard tables & columns standard\".",
  ...
  pt: {
    ...
    dashboardSingleView: "Painel único",
    dashboardTwoView: "Dois painéis",
    dashboardEmpty: "Ainda não há tabelas configuradas. Um painel pode conter qualquer número de tabelas; cada uma é declarada em `_data/config.ts` do projeto e renderizada através do padrão de tabela compartilhado — veja o README do projeto, \"The dashboard tables & columns standard\".",
  ...
  pl: {
    ...
    dashboardSingleView: "Jeden dashboard",
    dashboardTwoView: "Dwa dashboardy",
    dashboardEmpty: "Żadne tabele nie zostały jeszcze skonfigurowane. Dashboard może zawierać dowolną liczbę tabel; każda jest zadeklarowana w `_data/config.ts` projektu i renderowana przy użyciu wspólnego standardu tabel — zobacz README projektu, \"The dashboard tables & columns standard\".",
  ...
  tr: {
    ...
    dashboardSingleView: "Tek pano",
    dashboardTwoView: "İki pano",
    dashboardEmpty: "Henüz yapılandırılmış tablo yok. Bir pano herhangi bir sayıda tablo içerebilir; her biri projenin `_data/config.ts` dosyasında tanımlanır ve paylaşılan tablo standardıyla oluşturulur — bkz. proje README dosyası, \"The dashboard tables & columns standard\".",
  ...
  nl: {
    ...
    dashboardSingleView: "Eén dashboard",
    dashboardTwoView: "Twee dashboards",
    dashboardEmpty: "Nog geen tabellen geconfigureerd. Een dashboard kan een willekeurig aantal tabellen bevatten; elke tabel wordt gedeclareerd in `_data/config.ts` van het project en weergegeven via de gedeelde tabelstandaard — zie de README van het project, \"The dashboard tables & columns standard\".",
