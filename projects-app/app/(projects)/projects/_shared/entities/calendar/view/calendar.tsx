...
//
// THE CALENDAR — VIEW CORE (step 254.4). The read-only month grid + 30-min daily planner, reading the
// SAME generic rows store every other section uses (table id "calendar"; a row's `values` carry
// {date:"YYYY-MM-DD", time:"HH:MM", title, type:"event"|"reminder"}). Everything here is a READ act —
// a public visitor browses the calendar exactly like the owner. The demo-instruction banner and the
// requirement panel are ADMIN chrome (../admin, composed by ../index).
...
export function CalendarView({ automation }: { automation: string }) {
