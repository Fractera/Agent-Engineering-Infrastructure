...
import { messageOf, refuse } from "../message";
...
export async function deliverCalendar(ctx: NodeCtx): Promise<{ calendarRowId: string; calendarWhen: string }> {
  // parseDate не нашёл даты — честно спрашиваем «когда?», записи не создаём (её время неизвестно).
  if (ctx.needsWhen === true) refuse(needWhen);
