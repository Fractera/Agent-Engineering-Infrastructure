...
export default async function AutomationComponents({ lang, onboarding = false }: { lang: string; onboarding?: boolean }) {
  ...
  // ОНБОРДИНГ НОВОРОЖДЁННОГО (шаг 301): только что созданный замороженный клон стартера БЕЗ кейсов.
  // Продуктовые вкладки стартера (пульт, карта, календарь…) — это НЕ его автоматизация, а болванка:
  // показывать их владельцу, который ещё ничего не описал, — обещать работу, которой нет. Поэтому вместо
  // холста рисуем приглашение и сразу секцию кейсов, где рождается описание (Quiz). Как только появится
  // первый кейс, страница вернётся в обычный вид.
  if (onboarding) {
    return (
      <>
        <Welcome lang={lang} />
        <section data-section="use-cases" className="mt-8 border-t pt-6">
  ...
  const bodyOf = (tab: (typeof tabs)[number]) =>
    tab.name === "control-panel" ? (
      ...
      <Glossary entities={entitiesOf(tab)} lang={lang} />
    ) : tab.name === "assistant" ? (
