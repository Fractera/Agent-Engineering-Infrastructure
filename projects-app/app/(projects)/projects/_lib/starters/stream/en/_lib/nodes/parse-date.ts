...
import { refuse, servesIntent } from "../message";
...
export async function parseDate(ctx: NodeCtx): Promise<NodeCtx> {
  // Ветка НАПОМИНАНИЯ (308.8): дату разбираем только для намерения `remind`; иначе молчим. Backward-compat.
  if (!servesIntent(ctx, "remind")) return {};
