...
export type DiagramVMNode = {
  ...
  ioType: string | null;
  group: "input" | "intent" | "middle" | "speech" | "output" | "evolution";
  col: number; // столбец (0 вход · 1 намерение · 2 transform · 3 условие · 4 речь · 5 выход · 6 эволюция)
...
// РАЗГОВОРНЫЙ СЛОЙ (шаг 312) — группа `speech` между условиями середины и выходом: ответ человеку
// существует ДО того, как его повёз хоть один канал. Своя колонка, потому что до речи доходят и классы,
// минующие середину. Ядра без этой группы рисуются как прежде — колонки просто нет.
function colOf(group: "input" | "intent" | "middle" | "speech" | "output" | "evolution", kind: string): number {
  if (group === "input") return 0;
  if (group === "intent") return 1;
  if (group === "speech") return 4;
  if (group === "output") return 5;
  if (group === "evolution") return 6; // последний слой — работа автоматизации над собой, после доставки
...
export function graphToFlow(graph: Automation["graph"], components?: Automation["components"]): DiagramVM {
  ...
  for (const group of ["input", "intent", "middle", "speech", "output", "evolution"] as const) {
