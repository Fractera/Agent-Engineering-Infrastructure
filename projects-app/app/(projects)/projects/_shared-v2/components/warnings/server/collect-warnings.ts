import { coreEntitiesOf, type WarningRow, type WarningsCore } from "../types/warnings";
...
export function collectWarnings(core: WarningsCore): WarningRow[] {
  ...
  for (const tab of core.components.tabs) {
    ...
    for (const e of coreEntitiesOf(tab)) for (const w of e.warnings) out.push({ scope: "entity", name: e.name, cuid: w.cuid, text: w.text });
  }
