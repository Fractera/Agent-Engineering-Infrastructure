...
// источник), сама ничего не считает и не хранит. Стоит под статус-баром кокпита. Оранжевая — чтобы была
// заметной (правка владельца). Нет поводов → полосы нет вовсе.
//
// КНОПКА «ЗАПУСТИТЬ РАЗРАБОТКУ» диспатчит DOM-событие `fractera:launch-development`; запускатель уровня
// зоны ловит его и открывает консоль. ГЕЙТ: пока кейсы не подтверждены (нет повода `ready`) — кнопка
// приглушена; клик показывает причину, прокручивает к секции кейсов и открывает подтверждение.
        ...
        <ul className="space-y-1.5 border-t px-3 py-2 text-sm">
          {notices.map((notice, i) => (
            <li key={i} className="flex items-start gap-2">
              <span className="mt-0.5"><CatIcon category={notice.category} /></span>
