...
import { Check, Circle, ClipboardCopy, ClipboardPaste, Copy, Download, FlaskConical, Loader2, LogOut, Play, TerminalSquare } from "lucide-react";
...
import { ptyUrl } from "./pty-url";
import { XtermTerminal, type XtermTerminalHandle } from "./xterm-terminal.client";
import VoiceInput from "../../voice-input/client/voice-input.client";
...
export function DevConsole({
  ...
  // THE AUDIT DOWNLOAD (owner 2026-07-20) — asks the server for a freshly built room as a zip and lets the
  // browser save it. The task text the console is holding travels WITH the request, so the archive shows
  // the owner the exact words handed to this session, not a re-rendered approximation.
  const downloadRoom = async () => {
    setDownloading(true);
    try {
      const url = `/api/projects/room-archive?automation=${encodeURIComponent(automation)}&task=${encodeURIComponent(roomTask)}`;
      const res = await fetch(url);
      if (!res.ok) {
        const detail = await res.json().catch(() => null);
        toast.error(`${T.downloadFail}${detail?.error ? `: ${detail.error}` : ""}`);
        return;
      }
      ...
      a.href = href;
      a.download = `${automation.replace(/\//g, "-")}-room.zip`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(href);
    } catch (e) {
      toast.error(`${T.downloadFail}: ${String(e)}`);
    } finally {
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-3" data-dev-console="1">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* THE AUDIT BUTTON (owner 2026-07-20) — right of the session badge, in BOTH session states
              (new and reattached), because the owner audits the hand-off in both. It downloads the very
              room the agent works in plus the task text, as one zip. Reading only: no gate is bypassed
              and no development is started. */}
          <Button size="sm" variant="outline" onClick={downloadRoom} disabled={downloading} title={T.downloadTitle}>
      ...
      <Dialog open={pasteOpen} onOpenChange={setPasteOpen}>
        <DialogContent>
          ...
          <VoiceInput targetRef={pasteRef} value={pasteText} onChange={setPasteText} lang={lang} />
